...
counts <- fread("template_count_outside_gene.summarize.csv")
...
for (i in widths) {
    ...
    p_each_width3 <- ggplot() +
        geom_col(data = mean_counts[gene_extension_width == i], aes(sample_category, mean_intergenic_read_ratio)) +
        common_settings2 + xlab("Category") + ggtitle(sprintf("Gene extension length: %d", i))
    p_each_width <- c(p_each_width, list(p_each_width1, p_each_width2, p_each_width3))
}

cairo_pdf("template_count_outside_gene.summarize.plot.pdf", width = 4, height = 5, family = "Arial", onefile = TRUE)
print(p1)
print(p1_each)
print(p1_mean)
print(p_each_width)
