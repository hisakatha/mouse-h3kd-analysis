...
tad_strength_table = pd.DataFrame({"log10(Late 2-cell Control)": np.log10(tad_strength_control),
                                   ...
                                   "log2(Late 2-cell Control)": np.log2(tad_strength_control),
                                   "log2(Late 2-cell H3KD)": np.log2(tad_strength_h3kd),
                                   })
...
ax.set_aspect("equal", "box")
#ax.axis("equal")
hb = ax.hexbin(x="log2(Late 2-cell Control)", y="log2(Late 2-cell H3KD)", data=tad_strength_table,
          gridsize=33, edgecolors="none", bins=None, mincnt=1)
cbar = fig.colorbar(hb, label="Counts", shrink=0.8)
#cbar.set_ticks(np.unique(np.concatenate((cbar.get_ticks(), [1]))).tolist())
cbar.set_ticks([1,10,20,30,40,50])
sns.regplot(x="log2(Late 2-cell Control)", y="log2(Late 2-cell H3KD)", data=tad_strength_table,
            scatter=False, ax=ax, color="red")
ax.axline((0, 0), slope=1, color="black", linestyle="--")
ax.set_xlabel(r"$\log_{2}$(TAD strengths of Late 2-cell Control)")
ax.set_ylabel(r"$\log_{2}$(TAD strengths of Late 2-cell H3KD)")
fig.savefig("tad_strength.cor_h3kd_hex_log2.svg")
...
ax.set_box_aspect(4)
ax.set_xticks([])
ax.set_ylabel(r"$\log_{10}$(TAD str. of H3KD)/(TAD str. of Control)")
fig.savefig("tad_strength.log10_ratio_h3kd_box.svg")

fig, axs = plt.subplots(squeeze=False)
ax = axs[0, 0]
sns.histplot(log10_tad_strength_ratios, bins=np.arange(-1.6,1.6,0.1))
ax.set_xlabel(r"$\log_{10}$(TAD str. of H3KD)/(TAD str. of Control)")
ax.axvline(0, color="red", linestyle="--")
fig.savefig("tad_strength.log10_ratio_h3kd_hist.svg")

## log2
log2_tad_strength_ratios = np.log2(np.array(tad_strength_h3kd) / np.array(tad_strength_control))

# There is no differences between log10 and log2 for the signed rank test
tad_signed_rank = stats.wilcoxon(log10_tad_strength_ratios)
print(tad_signed_rank)
with open("tad_strength.log_ratio_h3kd_signed_rank.txt", "w") as f:
    f.write("Wilcoxon signed rank test: log Control vs. log H3KD\n"
            f"Statistic (two-sided): {tad_signed_rank.statistic}\n"
            f"P-value (two-sided): {tad_signed_rank.pvalue}\n"
            f"Mean of log10(H3KD/Control): {np.mean(log10_tad_strength_ratios)}\n"
            f"Median of log10(H3KD/Control): {np.median(log10_tad_strength_ratios)}\n"
            f"Mean of log2(H3KD/Control): {np.mean(log2_tad_strength_ratios)}\n"
            f"Median of log2(H3KD/Control): {np.median(log2_tad_strength_ratios)}\n"
            )

fig, axs = plt.subplots(squeeze=False)
ax = axs[0, 0]
sns.boxplot(log2_tad_strength_ratios)
ax.axhline(0, color="black", linestyle="--")
ax.set_box_aspect(4)
ax.set_xticks([])
ax.set_ylabel(r"$\log_{2}$(TAD str. of H3KD)/(TAD str. of Control)")
fig.savefig("tad_strength.log2_ratio_h3kd_box.svg")

fig, axs = plt.subplots(squeeze=False)
ax = axs[0, 0]
sns.histplot(log2_tad_strength_ratios, bins=np.arange(-5.5,5.5,0.5))
ax.set_xlabel(r"$\log_{2}$(TAD str. of H3KD)/(TAD str. of Control)")
ax.axvline(0, color="red", linestyle="--")
fig.savefig("tad_strength.log2_ratio_h3kd_hist.svg")
